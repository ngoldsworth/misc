...
if __name__ == "__main__":
    ...
    while remaining > 0:
        ...
        testchar = 9
        table_row = " {:>3} | ".format(m)
        table_row += "I: {:>{testchar}.2f} ({:>5.1%}) | ".format(
            interest, interest / payment, testchar=testchar
        )
        table_row += "P: {:>8.2f} ({:>5.1%}) | ".format(
            to_principle, to_principle / payment
        )
        table_row += "Extra: {:>8.2f} | ".format(extra_paid)
        table_row += "Tot {:>8.2f} | ".format(payment)
        table_row += "Remain: {:>8.2f} | ".format(remaining)
        table_row += "Cumul {:>10.2f} | ".format(total_paid)
